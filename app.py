...
# Pacific Time (automatically handles PDT / PST)
pacific_tz = pytz.timezone("America/Los_Angeles")
now_pacific = datetime.now(pacific_tz)
CURRENT_DATE = now_pacific.strftime("%B %d, %Y")
CURRENT_TIME = now_pacific.strftime("%I:%M %p %Z")

st.set_page_config(page_title="Oracle Nexus", page_icon="🔮", layout="wide")

st.title("🔮 Oracle Nexus")
st.caption(f"Your personal multi-persona AI with long-term memory • Today is {CURRENT_DATE} at {CURRENT_TIME}")

# Cache refresh button
if st.sidebar.button("🔄 Refresh Date & Clear Cache"):
    st.cache_data.clear()
    st.rerun()
...
Today's date and time in Pacific Time is {CURRENT_DATE} at {CURRENT_TIME}.
